...
# Function: a simple market making strategy
#########################################################################################
...
from .alphas import mean_reversion_ema, book_pressure

class MarketMakerBot(ServiceBot):
    """MarketMakerBot is a generic class for a market making strategy"""
    ...
    def generate_buyorder_size(self):
        """"""
    ...
    def generate_sellorder_size(self):
        """"""
        randomordersize = self.taskordersize * random.uniform(1.0-self.orderandomrange, 1.0+self.orderandomrange)
    ...
    def calculate_consolidated_alpha(self):
        ...
        alpha1 = mean_reversion_ema(midprice, self.ema_price) 
        alphas = [ alpha1 ]
        consolidated = alpha1 * self.alpha_weights[0]        
        if len(self.alpha_weights) >= 2:
            alpha = book_pressure(self.currentdepthbid, self.currentdepthask) 
            alphas.append(alpha)
            consolidated += alpha * self.alpha_weights[1]
        self.current_alpha = consolidated
        self.logger.debug("[calculate_consolidated_alpha]:: %s alpha_weight=%s, alphas=%s, consolidated=%s" % (self.tradingpair, str(self.alpha_weights), str(alphas), str(consolidated)) )
    ...
    def should_cancel_back(self, orderprice, ordersize, orderside):
        ...
        consolidated = self.current_alpha
    ...
    def should_cancel(self, orderprice, ordersize, orderside='BUY'):
        ...
        if self.should_cancel_back(orderprice, ordersize, orderside):
            return True
    ...
    def update_imbalance_from_order(self, order, orderdetails):
        if self.is_orderdetails_valid(orderdetails):
            orderside = order['orderside']
            order['lastAccumFilledAmount'] = order['accumFilledAmount']
            order['accumFilledAmount'] = float(orderdetails['FilledAmount'])
            if order['accumFilledAmount'] == order['ordersize']:
                order['inactive'] = 1
            self.update_imbalance(orderside, order['accumFilledAmount'] - order['lastAccumFilledAmount'], order['orderprice'])
            self.logger.debug(f"update imbalance from order :: {order['orderid']} {order['accumFilledAmount']} - {order['lastAccumFilledAmount']} = {order['accumFilledAmount'] - order['lastAccumFilledAmount']}, {order['orderprice']}")

    def update_imbalance(self, side, executed_qty, executed_price):
        ## account imbalance due to trading activity (in BTC term?)
        self.logger.debug(f"update imbalance :: before :: {self.imbalance_btc}")
        if side == "BUY":
            self.imbalance_btc = self.imbalance_btc - executed_qty * executed_price
        elif side == "SELL":
            self.imbalance_btc = self.imbalance_btc + executed_qty * executed_price
        self.logger.debug(f"update imbalance :: after :: {self.imbalance_btc}")
    ...
    def get_posting_price(self, orderside, numtierdepth = 0):
        ...
        if (self.current_mid() <= 0.0 or self.currentdepthbid[0]['price'] <=0.0 or self.currentdepthask[0]['price'] <= 0.0):
            ...
            return 0.0
        if orderside == 'BUY':
            price = round( min(self.current_mid() - posting_distance - numtierdepth * self.tierdepth, self.currentdepthbid[0]['price']), self.orderpricerounding)
            ...
            price = round( max(self.current_mid() + posting_distance + numtierdepth * self.tierdepth, self.currentdepthask[0]['price']), self.orderpricerounding)
    ...
    def generate_order(self, orderprice, ordersize, sellbroker=None, buybroker=None):
        ...
        if sellbroker:
            ...
            self.total_orders += 1
            self.logger.debug('generating sell orders %s' % str(limitsellorderID))
        if buybroker:
            limitbuyorderID = buybroker.client.buy_wait(self.tradingpair, orderprice, ordersize)[0]
            self.total_orders += 1
            self.logger.debug('generating buy orders %s' % str(limitbuyorderID))
    ...
    def create_single_tier(self,tiernumber,side,order_size=-1):
        ...
        if order_size > 0:
            ...
            ordersize = self.generate_buyorder_size()
        elif side == "SELL":
            ordersize = self.generate_sellorder_size()
        else:
    ...
    def is_orderdetails_valid(self, orderdetails):
        return isinstance(orderdetails, dict) and len(orderdetails) > 0 and orderdetails.get('FilledAmount',-1) >= 0
        ...
        for border in self.icebergOrderbuylist:
            ...
            if currentbuyorderid:
                ...
                if self.is_orderdetails_valid(buyorderdetails):
                    ...
                    border['accumFilledAmount'] = float(buyorderdetails['FilledAmount'])
                    ...
                    self.logger.debug(f"update imbalance from order :: {border['orderid']} {border['accumFilledAmount']} - {border['lastAccumFilledAmount']} = {border['accumFilledAmount'] - border['lastAccumFilledAmount']}, {border['orderprice']}")
        ...
        for sorder in self.icebergOrderselllist:
            ...
            if currentsellorderid:
                ...
                if self.is_orderdetails_valid(sellorderdetails):
                    ...
                    sorder['accumFilledAmount'] = float(sellorderdetails['FilledAmount'])
                    ...
                    self.logger.debug(f"update imbalance from order :: {sorder['orderid']} {sorder['accumFilledAmount']} - {sorder['lastAccumFilledAmount']} = {sorder['accumFilledAmount'] - sorder['lastAccumFilledAmount']}, {sorder['orderprice']}")
        ...
        if side == 'BUY':
            for border in self.icebergOrderbuylist:
                ...
                if self.is_orderdetails_valid(borderdetails):
                    self.brokers[0].client.cancel_wait(borderid, no_wait=True)
                    self.update_imbalance_from_order(border, borderdetails)
                    self.total_cancels += 1
                    left_amount = border['ordersize'] - float(borderdetails['FilledAmount'])
            ...
            for sorder in self.icebergOrderselllist:
                ...
                if self.is_orderdetails_valid(sorderdetails):
                    self.brokers[0].client.cancel_wait(sorderid, no_wait=True)
                    self.update_imbalance_from_order(sorder, sorderdetails)
                    self.total_cancels += 1
                    left_amount = sorder['ordersize'] - float(sorderdetails['FilledAmount'])
    ...
    def clear_and_cancel_by_alpha(self):
        for border in self.icebergOrderbuylist:
            ...
            if buyorderid:
                ...
                if self.is_orderdetails_valid(buyorderdetails):
                    left_amount = border['ordersize'] - float(buyorderdetails['FilledAmount'])
                    if buyorderdetails['Status'] == 'Completed' or buyorderdetails['Status'] == 'Canceled':
                        self.logger.info("[clear_and_cancel_by_alpha]:: order completed or canceled, orderid=%s" % buyorderid)
                        self.open_buy_orders_amount = max(0, self.open_buy_orders_amount - left_amount)
                        self.open_buy_orders_num = max(0, self.open_buy_orders_num - 1)
                        self.update_imbalance_from_order(border, buyorderdetails)
                        border['inactive'] = 1
                        continue
        ...
        for sorder in self.icebergOrderselllist:
            ...
            if sellorderid:
                ...
                if self.is_orderdetails_valid(sellorderdetails):
                    left_amount = sorder['ordersize'] - float(sellorderdetails['FilledAmount'])
                    if sellorderdetails['Status'] == 'Completed' or sellorderdetails['Status'] == 'Canceled':
                        self.logger.info("[clear_and_cancel_by_alpha]:: order completed or canceled, orderid=%s" % sellorderid)
                        self.open_sell_orders_amount = max(0, self.open_sell_orders_amount - left_amount)
                        self.open_sell_orders_num = max(0, self.open_sell_orders_num - 1)
                        self.update_imbalance_from_order(sorder, sellorderdetails)
                        sorder['inactive'] = 1
                        continue
    ...
    def cancel_for_chasing(self):
        ...
        if len(self.icebergOrderbuylist) == self.maxquotinglevels:
            ...
            if (new_buy_order_price - border0['orderprice']) > min_sep:
                ...
                borderdetails = self.brokers[0].client.get_order_details(self.tradingpair, 'BUY_LIMIT', borderid, data_format='legacy')
                if self.is_orderdetails_valid(borderdetails):
                    self.logger.info("[cancel_for_chasing]:: cancel - buy chase, orderid=%s" % borderid)
                    self.brokers[0].client.cancel_wait(borderid, no_wait=True)
                    self.update_imbalance_from_order(border1, borderdetails)
                    self.total_cancels += 1
                    left_amount = border1['ordersize'] - float(borderdetails['FilledAmount'])
                    self.open_buy_orders_amount = max(0, self.open_buy_orders_amount - left_amount)
                    self.open_buy_orders_num = max(0, self.open_buy_orders_num - 1)
                    border1['inactive'] = 1
        if len(self.icebergOrderselllist) == self.maxquotinglevels:
            ...
            if (new_sell_order_price - sorder0['orderprice']) < -min_sep:
                ...
                if self.is_orderdetails_valid(sorderdetails):
                    ...
                    self.brokers[0].client.cancel_wait(sorderid, no_wait=True)
                    self.update_imbalance_from_order(sorder1, sorderdetails)
                    self.total_cancels += 1
                    left_amount = sorder1['ordersize'] - float(sorderdetails['FilledAmount'])
                    self.open_sell_orders_amount = max(0, self.open_sell_orders_amount - left_amount)
                    self.open_sell_orders_num = max(0, self.open_sell_orders_num - 1)
                    sorder1['inactive'] = 1      
        self.clear_order_list()
    ...
    def cancel_for_risk(self):
        ...
        if self.imbalance_btc > self.imbalance_thresh:
            ...
            self.logger.info('imbalance btc threshold %s reached (at %s), cancel all sell orders.' % (str(self.imbalance_thresh), str(self.imbalance_btc)))
            self.cancel_on_side('SELL')
            self.open_sell_orders_amount = 0.0
            self.open_sell_orders_num = 0
        if self.imbalance_btc < -self.imbalance_thresh:
            self.logger.info('imbalance btc threshold %s reached (at %s), cancel all buy orders.' % (str(self.imbalance_thresh), str(self.imbalance_btc)))
    ...
    def do_market_making(self):
        ...
        self.logger.info('[do_market_making]:: %s%s imbalance_btc=%s' % (self.altcoin,self.quotecoin,str(self.imbalance_btc)))
        ...
        self.calculate_consolidated_alpha()
    ...
    def do_posting(self):
        ...
        while len(self.icebergOrderbuylist) < self.maxquotinglevels and dlvl < self.quotinglevels: 
            ...
            ## initial post
            if numOrders > 0:
                if any(abs(orderprice - iprice) < self.epsilon for iprice in pricelist): ## do not post if price level already there
                    dlvl += 1
            ...
            if numOrders == 0 or ins_index == 0 or (ins_index == numOrders and orderprice - pricelist[-1] > min_sep) or (orderprice - pricelist[ins_index-1] > min_sep and orderprice - pricelist[ins_index] < -min_sep) :
                ...
                if ordersize <= 0 or (minsize > 0 and ordersize < minsize) or self.should_cancel(orderprice, ordersize, 'BUY'):
                    dlvl += 1
                    self.logger.info('[do_posting]:: ordersize invalid or excessive, or BUY orderprice too aggressive')
                    continue
                sendorderid = self.generate_order(orderprice, ordersize, sellbroker=None, buybroker=self.brokers[0])
                self.logger.debug('[do_posting]:: prepare to send buy order price=%s, size=%s, orderid=%s' % (str(orderprice), str(ordersize), str(sendorderid)))
                if sendorderid is not None:
                    ...
                    self.logger.info('[do_posting]:: buy order sent: %s' % str(icebergorder))
            dlvl = dlvl + 1
        ...
        while len(self.icebergOrderselllist) < self.maxquotinglevels and dlvl < self.quotinglevels: 
            ...
            if numOrders == 0 or (ins_index == 0 and orderprice - pricelist[0] < -min_sep) or ins_index == numOrders or (orderprice - pricelist[ins_index-1] > min_sep and orderprice - pricelist[ins_index] < -min_sep) :
                ...
                if ordersize <= 0 or (minsize > 0 and ordersize < minsize) or self.should_cancel(orderprice, ordersize, 'SELL'):
                    dlvl += 1
                    self.logger.info('[do_posting]:: ordersize invalid or excessive, or SELL orderprice too aggressive')
                    continue
                sendorderid = self.generate_order(orderprice, ordersize, sellbroker=self.brokers[0], buybroker=None)
                self.logger.debug('[do_posting]:: prepare to send sell order price=%s, size=%s, orderid=%s' % (str(orderprice), str(ordersize), str(sendorderid)))
                if sendorderid is not None:
                    icebergorder = {"tiernumber": dlvl,
                                    "ordersize": ordersize,
                                    "orderprice":orderprice,
                    ...
                    self.logger.info('[do_posting]:: sell order sent: %s' % str(icebergorder))
            dlvl = dlvl + 1
    ...
    def update_order_history(self, dump_to_csv = False):
        if not self.clerk:
            return
        records = self.clerk.get_record_from_db(self.tradingpair, self.brokers[0].get_exchangename(), self.brokers[0].get_account_id(), source='Market')
    ...
    def get_latest_orders(self, limit=10, filename=None):
        results = []
        if self.clerk:
            dataset = self.clerk.get_record_from_db(self.tradingpair, self.brokers[0].get_exchangename(), self.brokers[0].get_account_id(), num=10, status=['Completed','Not Completed','Canceled'], source='Market', botid=self.botid)
        for data in dataset:
    ...
    def record_order_to_csv(self, orderdetails):
        ...
        if isinstance(orderdetails, dict) and len(orderdetails) > 0:
            filename = self.altcoin.upper() + '_' + self.quotecoin.upper() + '_' + self.brokers[0].get_exchangename() +'_Market_log.csv'
